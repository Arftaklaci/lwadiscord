...
async function main(){
  ...
  console.log('Token Oluşturucu ArifReis');
  ...
  if(choice == 1){
    console.log('['+'1'.brightBlue+'] Temp-mail Önerim');
    console.log('['+'2'.brightBlue+'] 10minemai');
    console.log('['+'3'.brightBlue+'] Tempmaildev');
    let emailchoice = prompt('[?]>');
    console.log('['+'!'.red+'] Tokenleri WebHook a göndermek istiyormusun (Y=evet,N=Hayır) y/n: ');
    let choicewbwile = prompt('[?]>');
    if(choicewbwile == 'y'){
      webhook = prompt('Webhook: ')
    }
    console.log('['+'!'.green+'] Tokenleri Sunucuya a göndermek istiyormusun (Y=evet,N=Hayır) y/n: ');
    let choicewbwile2 = prompt('[?]>');
    if(choicewbwile2 == 'y'){
      sunucudavet = prompt('Sunucu Davet Bağlantısı: ')
    }
    let tokensname = prompt('Tokens username: ( Boş Bırakırsan	Kendisi Random Yapıyor ) ');
    ...
    async function discordInput(dspagee, username, password, email){
      ...
      await dspagee.goto('https://discord.com/register', {"waitUntil" : "networkidle0", timeout: 100000});
    ...
    async function create_accinfos(chrome, disc) {
      const username = uniqueNamesGenerator({dictionaries: [animals, nickname],  separator: ' ', style: "capital",length: 2,});
      const password = "kaanarifsamir" //Hesapların Şifresi
    ...
    console.log('Yardım almak için ArifReisi Bul :)')
    ...
    console.log('YARAK KAFA YANLIS TIKLADIN'.red);
    await sleep(1000);
    main()
  }
}
